...
interface CommunityWizardProps {
  ...
  onCancel: () => void;
  embedded?: boolean; // If true, don't render Dialog wrapper (for use inside OrganizationWizard)
}
...
export function CommunityWizard({ onComplete, onCancel, embedded = false }: CommunityWizardProps) {
  ...
  const innerContent = (
    <>
        <DialogHeader className={embedded ? "p-8 pb-6" : ""}>
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="text-2xl">AI Structure Wizard</DialogTitle>
        ...
        <div className="flex-1 overflow-y-auto">
          {currentStep === 1 && (
            <div className={`space-y-6 ${embedded ? "px-8 py-6" : "p-1"}`}>
              ...
              <div className="space-y-3">
                <Label>Channel Budget: {data.channelBudget[0]}</Label>
                <Slider
                  value={data.channelBudget}
                  onValueChange={(value) => setData(prev => ({ ...prev, channelBudget: value }))}
                  min={4}
                  max={20}
                  step={1}
                  className="w-full"
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) => setData(prev => ({ ...prev, importWorkspace: checked as boolean }))}
                    />
          ...
          {currentStep === 3 && (
            <div className={`space-y-6 ${embedded ? "px-8 py-6" : "p-1"}`}>
              ...
              <div className="space-y-4">
                ...
                <Card>
                  <CardContent className="pt-4">
                    <div className="text-sm text-muted-foreground">Channel Budget</div>
        ...
        <div className={`flex justify-between border-t ${embedded ? "px-8 py-6 mt-auto" : "pt-4"}`}>
  ...
  // If embedded in another wizard, render inner content only
  if (embedded) {
    return (
  ...
  // Otherwise wrap in Dialog with DialogContent
  return (
    <Dialog open={true} onOpenChange={() => {}}>
      <DialogContent className="max-w-[720px] h-[620px] flex flex-col bg-card text-card-foreground border-border">
        {innerContent}
      </DialogContent>
    </Dialog>
  );
